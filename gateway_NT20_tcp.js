...
var server = net.createServer((socket) => {
   var nt20 = new protocolGT06('nt20');
   //console.log('Mais um equipamento conectado.');

   socket.on("error", (err) => {
      const fileName = 'log_nt20.txt';
   ...
   socket.on('data', (data) => {
      ...
      if (nt20.msgBuffer.length > 0) {
         ...
         if (nt20.expectsResponse) {
            socket.write(nt20.responseMsg);
         }      

         nt20.msgBuffer.forEach((msg, idx) => {
         ...
         //bdados.interacaoDBase(nt20.msgBuffer);  
